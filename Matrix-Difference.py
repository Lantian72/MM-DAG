...
def multiDAG_functional(X, lambda1, rho, P_id, P_all, max_iter=200, alpha_max=1e+16, G_true=None):
    def _adj(gt):
        G = {}
        iter = 0
        for l in range(L):
                    ...
                    for ni in range(P_all):
                        for nj in range(P_all):
                            ...
                            if (ni in re_id[l]) and (nj in re_id[l]) and (ni in re_id[_l]) and (nj in re_id[_l]):
                                ...
                                dis = (fG[l][li, lj] - fG[_l][_li, _lj]) ** 2 * rho
                                p += dis
                                G_pG[l] += 2 * dis * G_fG[l][li, lj, :, :]
    ...
    def _func(gt):
        ...
        # g_loss = p
        for l in range(L):
            G_Gsmooth1[l] = G_Gloss[l] + (alpha * h + beta) * G_hG[l] + lambda1 + G_pG[l]
            G_Gsmooth2[l] = -(G_Gloss[l] + (alpha * h + beta) * G_hG[l]) + lambda1 - G_pG[l]
        g_grad = _vec(G_Gsmooth1, G_Gsmooth2)
    ...
    def _train(optimizer):
        for _ in range(1000):
            ...
            # f_now, _ = _func(optimizer.params)
            # f_next, _ = _func(optimizer.params + 0.00001 * _grad(optimizer.params))
            # if f_next < f_now:
            # print(f_next, f_now)
    ...
    bounds = _bounds()
    gt_est = _vec(Empty, Empty)
    alpha, beta, h = 1, 1, np.inf
    if G_true is not None:
        gt_true = _vec(G_true, Empty)
        _debug(gt_true)
    ...
    G_est = _adj(gt_est)
    E_est = {}
    # optimizer = Adam(gt_est=_vec(G_est))
    # gt_new = _train(optimizer)
    # G_est = _adj(gt_new)
    for l in range(L):
...
def single_test(N=None, K=None, P=None, n_task=None, rep=None):
    ...
    for rep_id in range(rep):
        ...
        a, v = net_fpca(nh, K)
        E_est, G_est = multiDAG_functional(a.copy(), lambda1=np.log(P) / N / 2, rho=2 / n_task, P_id=npid, P_all=P)
        for l in range(n_task):
            np.savetxt('./result/graph/miss/E_est_%i,task_%i.csv' % (n_task, l + n_task * rep_id), E_est[l])
            np.savetxt('./result/graph/miss/W_true_%i,task_%i.csv' % (n_task, l + n_task * rep_id), W_true[l + rep_id * n_task])
            E_est[l][E_est[l] < 0.5] = 0
            E_est[l][E_est[l] > 0.1] = 1
            acc = utils.count_accuracy(B_true=E_true[l + rep_id * n_task], B_est=E_est[l])
            result_pd.loc[niter] = [N, K, P, l, acc['fdr'], acc['tpr'], 'MULTITASK-MISS', rep_id]
            niter += 1
    result_pd.to_csv('./result/miss/5N=%i,K=%i,P=%i,L=%i.csv' % (N, K, P, n_task))
